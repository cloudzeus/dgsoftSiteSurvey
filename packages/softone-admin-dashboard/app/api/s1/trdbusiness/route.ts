...
export async function GET(req: Request) {
  ...
  const businesses = rows
    .filter((r: Record<string, unknown>) => r.TRDBUSINESS != null && r.NAME != null)
    .map((r: Record<string, unknown>) => ({ id: Number(r.TRDBUSINESS), name: String(r.NAME) }))
