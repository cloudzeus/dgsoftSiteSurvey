...
const DEFAULT_COL_WIDTHS: Record<string, number> = { name: 320, category: 160 }
...
function AddAssetTypeDialog({ onSuccess }: { onSuccess: (t: AssetTypeRow) => void }) {
  const [open, setOpen]         = useState(false)
  const [name, setName]         = useState("")
  const [category, setCategory] = useState<MasterCategory>("COMPUTING")
  const [error, setError]       = useState("")
  const [pending, start]        = useTransition()

  function reset() { setName(""); setCategory("COMPUTING"); setError("") }
  function close() { reset(); setOpen(false) }

  function submit() {
    if (!name.trim()) { setError("Name is required"); return }
    start(async () => {
      const res = await createAssetType({ name, category })
      if (res.error) { setError(res.error); return }
      onSuccess(res.assetType!)
      close()
    })
  }
  ...
  return (
    <>
      <Btn variant="primary" size="sm" onClick={() => setOpen(true)}>
        <Plus className="size-3.5 mr-1" />Add asset type
      </Btn>
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4"
          ...
          <div className="modal-card w-full max-w-sm space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-[15px] font-semibold" style={{ color: "var(--foreground)" }}>Add asset type</h2>
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <input value={name} onChange={e => setName(e.target.value)}
                  placeholder="e.g. Managed Switch" className="input-field"
...
function EditDialog({ assetType, onSuccess, onClose }: {
  assetType: AssetTypeRow; onSuccess: (t: AssetTypeRow) => void; onClose: () => void
}) {
  const [name, setName]         = useState(assetType.name)
  const [category, setCategory] = useState<MasterCategory>(assetType.category)
  const [error, setError]       = useState("")
  const [pending, start]        = useTransition()
  ...
  function submit() {
    if (!name.trim()) { setError("Name is required"); return }
    start(async () => {
      const res = await updateAssetType(assetType.id, { name, category })
      if (res.error) { setError(res.error); return }
      onSuccess(res.assetType!)
      onClose()
    })
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4"
      ...
      <div className="modal-card w-full max-w-sm space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="text-[15px] font-semibold" style={{ color: "var(--foreground)" }}>Edit asset type</h2>
...
export function AssetTypesTable({ initialAssetTypes }: { initialAssetTypes: AssetTypeRow[] }) {
  const [assetTypes, setAssetTypes] = useState<AssetTypeRow[]>(initialAssetTypes)
  ...
  const { visibleCols, toggleCol, pageSize, setPageSize, hydrated } =
    useTablePrefs("asset-types", COLUMNS, 25, DEFAULT_COL_WIDTHS)
  ...
  }, [assetTypes, search, sortKey, sortDir])
  ...
  function handleDelete(id: number) {
    start(async () => {
      ...
      if (!res.error) setAssetTypes(prev => prev.filter(t => t.id !== id))
    })
  }
  ...
  function handleBulkDelete() {
    ...
    start(async () => {
      const res = await deleteAssetTypes(ids)
      if (!res.error) { setAssetTypes(prev => prev.filter(t => !ids.includes(t.id))); setSelected(new Set()) }
    })
  }
  ...
  return (
    <>
      <div className="rounded-xl overflow-hidden"
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-3"
          ...
          <div className="flex items-center gap-2">
            ...
            <AddAssetTypeDialog onSuccess={t => { setAssetTypes(prev => [t, ...prev]); setPage(1) }} />
          </div>
        </div>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={visibleColDefs.length + 2}
                    ...
                    {search ? "No asset types match your search" : "No asset types yet — add one above"}
                  </td>
                </tr>
              ) : pageRows.map((item, i) => {
                    ...
                    <td className="pl-4 pr-2 py-3.5">
                      <RowCheckbox checked={isSel}
                        onCheckedChange={v => setSelected(prev => {
                          const n = new Set(prev); v ? n.add(item.id) : n.delete(item.id); return n
                        })} />
                    </td>
                    {visibleCols.has("name") && (
                      <td className="px-4 py-3.5 font-medium truncate"
                        style={{ color: "var(--foreground)" }}>{item.name}</td>
                    )}
      ...
      {editing && (
        <EditDialog
          assetType={editing}
          onSuccess={updated => setAssetTypes(prev => prev.map(t => t.id === updated.id ? updated : t))}
